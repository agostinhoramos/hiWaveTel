"""Probe modem/SIM identity via mmcli for automatic HiDishelinkDevice bootstrap."""
...
def _extract_own_number(details: dict[str, str]) -> str:
    ...
    if not raw:
        return ''
    # mmcli may return "351912329317" or "+351912329317" or comma-separated list
...
def format_modem_notes(identity: dict[str, Any], *, probed_at: str) -> str:
    """Human-readable notes line for HiDishelinkDevice admin."""
    parts = [
        f'manufacturer={identity.get("manufacturer") or "unknown"}',
        f'model={identity.get("model") or "unknown"}',
        f'imei={identity.get("imei") or "unknown"}',
        f'firmware={identity.get("firmware") or "unknown"}',
        f'modem_index={identity.get("modem_index", "?")}',
        f'probed_at={probed_at}',
    ]
    return 'Auto-detected modem: ' + ', '.join(parts)
