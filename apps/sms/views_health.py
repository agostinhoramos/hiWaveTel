...
def health_modem_manager(request: HttpRequest) -> JsonResponse:  # noqa: ARG001
    ...
    try:
        ...
        if configured not in indices:
            payload['mmcli_notes'] = (
                f'MODEM_MMCLI_INDEX={configured} not in {indices}; align AUTO_DETECT or set it manually.'
            )
            return JsonResponse(payload, status=503)

        ping_ok, ping_text = client.modem_ping(int(configured))
