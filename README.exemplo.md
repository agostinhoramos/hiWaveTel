...
**Painel Django Admin (`/admin/`):** no Docker o processo usa **Gunicorn**, que não serve estáticos por defeito; o projecto usa **WhiteNoise** e `collectstatic` na imagem. Se o login do admin aparecer sem CSS, reconstrua a imagem (`docker compose build`) e confira que `GET /static/admin/css/base.css` responde **200**.
...
| `MQTT_EXTERNAL_TOPIC_PREFIX` | Base por defeito para modems quando `MQTT_BASE_TOPIC_PREFIX` está vazio (ex.: `hidishelink_dev`). |
| `MQTT_BASE_TOPIC_PREFIX` | Prefixo para `…/modems/…`; por defeito igual a `MQTT_EXTERNAL_TOPIC_PREFIX`. |
| `MQTT_DEVICE_TOPIC_PREFIX` | Prefixo completo antes de `/{id}/sms/…` (hiDisheLink); por defeito `{MQTT_BASE_TOPIC_PREFIX}/devices`. Valores `${MQTT_BASE_TOPIC_PREFIX}/devices` são expandidos em `base.py`. |
| `MQTT_PUBLISH_SEND_REQUEST` | `true` por defeito: após cada `POST …/sms/send/` o gateway publica também o JSON em `{MQTT_DEVICE_TOPIC_PREFIX}/…/sms/send` no broker |
...
| `MQTT_MODEM_INBOX_DELIVERY_MODE` | `broadcast` (**defeito**): uma publicação canónica em `{modem_prefix}/modems/{modem_index}/sms/inbox_delivery` com `message_id` tipo `mmcli_<pk>`, listas opcionais `mirrored_device_ids` e mapa `device_message_ids`. `per_device`: `{device_prefix}/{id_sanitizado}/sms/inbox_delivery`, um publish por `ExternalDevice` que espelhou a mesma entrada. |
| `MQTT_MODEM_STATUS_SUBSCRIBE` | `true` por defeito: o gateway subscreve `{modem_prefix}/modems/+/status/request` e responde em `…/status/response` com JSON de estado do modem (`mmcli`). Defina `false` para não subscrever (útil quando só se usa o modo efémero de inbox/send). |
...
| `MQTT_MODEM_STATUS_AUTO_PUBLISH` | `true` por defeito: ligado ao broker, o gateway enumera os modems com `mmcli -L`, publica o snapshot em `{modem_prefix}/modems/N/status/telemetry` no arranque (e após cada reconexão bem sucedida) e opcionalmente em mudanças de estado (polling). |
...
Prefixos: **`device_prefix`** = `MQTT_DEVICE_TOPIC_PREFIX` efectivo (tipicamente `{MQTT_EXTERNAL_TOPIC_PREFIX}/devices`), **`modem_prefix`** = `MQTT_BASE_TOPIC_PREFIX` efectivo (tipicamente igual a `MQTT_EXTERNAL_TOPIC_PREFIX`). O `GET /api/sms/device/mqtt-config/` no hiWaveTel **proxifica** o hiDisheLink (`HIDISHELINK_API_URL` ou credenciais por dispositivo no Admin); o JSON `data` espelha o remoto (`TOPIC_*` com `{device_id}`, etc.).
...
- `{device_prefix}/+/sms/status`
- `{device_prefix}/+/sms/inbox`
- `{modem_prefix}/modems/+/status/request` quando `MQTT_MODEM_STATUS_SUBSCRIBE` é verdadeiro (**defeito**)

**O gateway publica:**

- `{device_prefix}/{device_id_sanitizado}/sms/send` (notificação de pedidos de envio originados pela API HTTP; mesmo contrato §3.3)
- `{device_prefix}/{device_id_sanitizado}/sms/inbox/ack`
- Com `MQTT_PUBLISH_MODEM_INBOX=true`:
  - **Modo recomendado** (`MQTT_MODEM_INBOX_DELIVERY_MODE=broadcast`, **defeito**): `{modem_prefix}/modems/{modem_index}/sms/inbox_delivery` — uma cópia canónica da SMS quando o espelho da inbox ficou útil aos subscritores (ver §3.4).
  - **Modo legado** (`per_device`): `{device_prefix}/{device_id_sanitizado}/sms/inbox_delivery` — até N publicações (uma por dispositivo activo sem filtro de modem compatível).

- Telemetria (não solicitada): `{modem_prefix}/modems/{modem_index}/status/telemetry` com `event=bootstrap` após cada ligação ao broker bem sucedida, e `event=state_change` quando `mmcli` reporta dados alterados (ver `MQTT_MODEM_STATUS_POLL_INTERVAL_SEC`).
- Pedido sob demanda: `{modem_prefix}/modems/{modem_index}/status/response` como resposta aos pedidos §3.5.
...
mosquitto_pub -h localhost -p 1883 \
  -t "hidishelink_dev/devices/351913000001/sms/status" \
...
```bash
mosquitto_pub -h localhost -p 1883 \
  -t "hidishelink_dev/devices/351913000001/sms/inbox" \
...
mosquitto_sub -h localhost -p 1883 \
  -t "hidishelink_dev/devices/351913000001/sms/send" -v
```
...
**Modo `broadcast` (defeito):** `{modem_prefix}/modems/{modem_index}/sms/inbox_delivery` — payload JSON típico: `message_id` (`mmcli_<pk>` da `InboundSms`), `sender`, `body`, `received_at`, `modem_index`, e opcionalmente `mirrored_device_ids` / `device_message_ids` (`mmcli_<pk>_dev_<ExternalDevice.pk>` por dispositivo).

**Modo `per_device`:** `{device_prefix}/{id_sanitizado}/sms/inbox_delivery`, com `message_id`=`mmcli_<pk>_dev_<ExternalDevice.pk>`.
